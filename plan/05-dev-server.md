...
把現有的 dev_server.py 整合進 CLI，加入自動化功能。具體工作：

1. 整合進 CLI：`rayport dev ./my-game/`
2. 檔案監控：偵測 .py 檔或資源檔變動，自動重新打包 tar.gz
3. 瀏覽器自動重新載入：透過 WebSocket 或輪詢通知瀏覽器重新載入遊戲
4. 預設開啟瀏覽器

此模組是第四階段，依賴第三階段（模組 2 + 4）完成。
